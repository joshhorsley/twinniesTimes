...
export default function MemberPlot({ plotData, dataOption }) {
  ...
  // Update xrangeOptions
  useEffect(() => {
    setXRangeOptions(
      plotData.xRangeOptions[0].map((e) => {
        return {
          label: e.label[0],
          value: e.value[0],
        };
  ...
  useEffect(() => {
    if (
      !plotData.barData.length &
      !plotData.barDataManual.length &
      !plotData.marshalling.length
    ) {
      ...
      if (plotData.barDataManual.length) {
        data3 = plotData.barDataManual.map((e) => {
          ...
          const baseZero = e.x.map((e, i) => {
            return 0;
          });
          const yDefault = e.x.map((e, i) => {
            return 5400;
          });
          ...
          return {
            ...
            hovertemplate:
              "%{customdata.dateNice}<br><b>%{data.name}</b><br>Not timed<br><extra></extra>",
          };
        });
      }
  ...
  useEffect(() => {
    if (dataOption == "all") {
      ...
      const dataSubset = dataMember.filter(
        (e) => e.customdata[0].distance == dataOption
      );
