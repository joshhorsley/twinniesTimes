...
export default function RacePlot2({ plotData, distance, category }) {
  ...
  useEffect(() => {
    ...
    if (annotationsFiltered.length) {
      if (Object.hasOwn(annotationsFiltered[0], "annotations")) {
        annotationsPrep = annotationsFiltered[0].annotations.name_display.map(
          (e, i) => {
            const annotationProps =
              annotationsFiltered[0].annotations.points_handicap_display.map(
                (e, i) => {
                  ...
                  return e == 0
                    ...
                    : {
                        text:
                          " " +
                          annotationsFiltered[0].annotations.timeDiff[i] +
                          "s → " +
                          e +
                          "pts",
                        color: "green",
            ...
            return {
              text:
                annotationsFiltered[0].annotations.name_display[i] +
                annotationProps[i].text,
              x: annotationsFiltered[0].annotations[displayModeX][i] + 90,
              y: annotationsFiltered[0].annotations[displayModeY][i],

              font: { size: "13", color: annotationProps[i].color },
    ...
    if (layoutFiltered.length) {
      if (Object.hasOwn(layoutFiltered[0], "xaxis")) {
        xaxisPrep = {
          ...
          ticktext:
            displayModeX == "x0Clock"
              ? layoutFiltered[0].xaxis.ticktext
              : layoutFiltered[0].xaxis.ticktextZero,
          side: "top",
        };
      }
