...
export default function PeopleTable({ dataPeople }) {
  ...
  return (
    ...
    >
      <Column field="seasonDisplay" header="Season" sortable frozen />

      {dataPeople.tab.colDefs.map((col) => (
        <Column
          key={col.columnID}
          // field="season"
          header={col.title}
          body={(e) => {
            const tags = e[`${col.title}`].map((e2, i) => {
              return (
                <Link key={`${col.title}-${i}`} to={`/members/${e2.id_member}`}>
                  {e2.name_display}
                </Link>
              );
            });
